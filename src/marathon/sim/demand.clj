...
Public Sub registerDemand(demand As TimeStep_DemandData, demandstore As TimeStep_ManagerOfDemand, policystore As TimeStep_ManagerOfPolicy, ctx As TimeStep_SimContext)
...
'
'
Public Sub addFillable(fillrule As String, demandstore As TimeStep_ManagerOfDemand)
...
Public Sub removeFillable(fillrule As String, demandstore As TimeStep_ManagerOfDemand)
...
Public Sub FillDemands(t As Single, Optional state As TimeStep_SimState)
...
'TOM Change 7 Sep 2012
'FillFollowOns t, parent.SupplyManager.followonbuckets 'ensures that follow-on eligible demands exhaust follow-on supply, in priority order.

MarathonOpDemand.FillFollowOns t, state.fillstore, state.supplystore, _
                        state.parameters, state.demandstore, state.policystore, state.context
'FillFollowOns t, state.supplystore.followonbuckets

'TOM Change 17 Aug 2012
'parent.SupplyManager.ReleaseMaxUtilizers
'TOM Change 7 Sep 2012
MarathonOpSupply.ReleaseMaxUtilizers state.supplystore, state.context

'TOM Change 7 Sep 2012
MarathonOpDemand.FillNormalDemands t, state.fillstore, state.supplystore, state.parameters, _
                                        state.demandstore, state.policystore, state.context
'FillNormalDemands t 'fills remaining demands, expanding supply beyond follow-on supply.
...
Public Sub FillFollowOns(t As Single, fillstore As TimeStep_ManagerOfFill, _
...
Private Function getFollowOnDemands(demandstore As TimeStep_ManagerOfDemand, followonbuckets As Dictionary, unfilledDemands As Dictionary) As GenericSortedDictionary
Dim dem
Dim demand As TimeStep_DemandData
...
Public Sub scopeDemand(demandstore As TimeStep_ManagerOfDemand, disableTags As Dictionary, Optional removal As Boolean)
...
Public Sub removeDemand(demandstore As TimeStep_ManagerOfDemand, demandname As String)
...
End Sub
Public Function isEnabled(demandstore As TimeStep_ManagerOfDemand, demandname As String) As Boolean
isEnabled = demandstore.tags.hasTag("Enabled", demandname)
End Function
Public Sub enable(demandstore As TimeStep_ManagerOfDemand, demandname As String)
demandstore.tags.addTag "Enabled", demandname
End Sub
Public Sub disable(demandstore As TimeStep_ManagerOfDemand, demandname As String)
...
Public Sub AddActivation(t As Single, demandname As String, demandstore As TimeStep_ManagerOfDemand, ctx As TimeStep_SimContext, Optional activationdict As Dictionary)
