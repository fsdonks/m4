...
''->Re: the below discussion on a declarative specification for defining composable
'periods....We can simple bifurcate periods into two classes:
...
Public Function namedPeriod(name As String, Optional p As GenericPeriod) As GenericPeriod
If Not (exists(p)) Then Set p = New GenericPeriod
p.name = name
Set namedPeriod = p
End Function
'General period constructor.  Defines a period [tstart...tfinal], where tstart <= tfinal
Public Function periodAcross(tstart As Single, tfinal As Single) As GenericPeriod
Set periodAcross = New GenericPeriod
If tfinal < tstart Then Err.Raise 1010, , "Periods must be defined so that tfinal is >= tstart"
periodAcross.fromday = tstart
periodAcross.today = tfinal
End Function
'Defines a period whose only valid intersection value is tstart, [tstart..tstart]
'Used to define instantaneous phenomena...
Public Function periodAt(tstart As Single) As GenericPeriod
Set periodAt = periodAcross(tstart, tstart)
End Function
'Defines a negatively unbounded period over [-inf...tfinal]
Public Function periodTo(tfinal As Single) As GenericPeriod
Set periodTo = periodAcross(infnegative, tfinal)
End Function
'Defines a positively unbounded period over [tstart...inf]
Public Function periodFrom(tstart As Single) As GenericPeriod
Set periodFrom = periodAcross(tstart, inf)
End Function
'Defines a period that cannot ever intersect with any value.
Public Function periodUndefined() As GenericPeriod
Set periodUndefined = periodAcross(inf, inf)
End Function
'Defines a period that intersects every value of t [-inf...inf]
Public Function periodInfinite() As GenericPeriod
Set periodInfinite = periodAcross(infnegative, inf)
End Function
Public Function periodToList(p As GenericPeriod) As Collection
Set periodToList = list(p.fromday, p.today, p.name)
End Function
Public Function listToPeriod(l As Collection) As GenericPeriod
Set listToPeriod = periodAcross(fst(l), snd(l))
End Function

'Simple 1 dimensional intersection function.  Allows for infinite values in either direction, i.e.
'[tstart...inf], or [-inf...tfinal] are valid.  Returns false if t is inf.
Public Function intersect1D(t As Single, tstart As Single, tfinal As Single) As Boolean
intersect1D = False

If t = inf Then
    intersect1D = False
ElseIf (tstart = infnegative) And (tfinal = inf) Then
    intersect1D = True
Else
    intersect1D = (t >= tstart) And (t <= tfinal)
End If

End Function
'Determines if time t intersects the period p
Public Function intersectsPeriod(t As Single, p As GenericPeriod) As Boolean
intersectsPeriod = intersect1D(t, p.fromday, p.today)
End Function
...
Public Function recordToPeriod(inrec As GenericRecord) As GenericPeriod
With inrec
    Set recordToPeriod = PeriodLib.namedPeriod(.fields("Name"), _
                            PeriodLib.periodAcross(parseInf(.fields("FromDay")), parseInf(.fields("ToDay"))))
End With

End Function

'Public Function makeTemporalPeriod(Optional startday As Long, Optional endday As Single, Optional periodName As String) As IPeriodic
'
'Set makeTemporalPeriod = New GenericPeriod
'
'If periodName = "" Then periodName = "Default"
'
'With makeTemporalPeriod
'    .name = periodName
'    .fromday = startday
'
'    If .endday < 0 Then Err.Raise 101, , "Cannot have a negative end day for a period"
'
'    .today = endday
'End With
'
