...
(ns marathon.bridgelib.bridging
...
(defn make-bridge
  ...
  (let [bridge 
    ...
    (zipmap (map get-source bridge) (map get-target bridge))))
...
(defn translate-map
  ...
  [bridgemap m]  
  (let [fields (into #{} (keys m))
        bridges (into #{} (keys bridgemap))
        mapped-fields (setlib/intersection fields bridges)
        static-fields (setlib/difference bridges mapped-fields)
        static-map (into {} (for [k static-fields]
                                 [k (fnext (get bridgemap k))]))
        re-map (fn [k] 
                    (let [[tgt _] (get bridgemap k)]
                      [tgt (get m k)]))]    
    (into {} (for [k bridges] ;preserves original order...
               (if (static-fields k) 
                   [k (get static-map k)]
                   (re-map k))))))
(comment
...
;        
;    (merge (->> mapped-fields
;                (reduce (fn [acc k] (conj acc  (re-map k))) {}))
;           static-map)))