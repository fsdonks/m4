;Defines 2 new data bridges for Megan Malone's demand and supply files from
;BAF.
(ns marathon.bridgelib.binadapter
  (:require [marathon.bridgelib [bridging :as bridging]]))
...
(bridging/defbridge BAF-supply-bridge  
  [["SRC"] 
   ["STR"]
   ["Component"]
   ["Pool"]])
...
(bridging/defbridge BAF-demand-bridge 
  [["QTY" "Quantity" 0]
   ["Start" "StartDay" 0]
   ["Duration" "Duration" 0]
   ["TAA SRC"  "SRC"]
...
(def bridged-supply 
  ;(let [bridge (bridging/make-bridge supply-template)]
  (map (partial bridging/translate-map BAF-supply-bridge) supply))

(def bridged-demand 
  (map (partial bridging/translate-map BAF-demand-bridge) demand))
)
